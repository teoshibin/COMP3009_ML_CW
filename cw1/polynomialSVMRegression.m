    ...
    for j = 1:inner_k
       ...
       for order = most_order_boundary
        ...
        fprintf("\t Inner: %d TestSize: %d TrainSize: %d " + ...
                "RMSE: %f  BestPolyOrder: %d BestEpsilon: %d\n", ...
                j, height(inner_test_data), height(inner_train_data), ...
                bestRMSE,  all_best_hyper_order(i,j), all_best_epsilon(i,j));
    end
...
fprintf("FinalRMSEPolynomial: %f FinalOrder: %d FinalEpsilon: %d", ...
      mean_RMSE_polynomial, best_hyper_order, best_hyper_epsilon);
